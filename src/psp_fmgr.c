...
psp_fmgr_menu(int format, int disk_id)
{
  ...
  if (first) {
    first = 0;
    sprintf(user_filedir_kbd, "%s/kbd/", THOM.thom_home_dir);
    sprintf(user_filedir_set, "%s/set/", THOM.thom_home_dir);
    sprintf(user_filedir_snap, "%s/snap/", THOM.thom_home_dir);
    sprintf(user_filedir_k7, THOM.thom_k7_dir);
    sprintf(user_filedir_disk, THOM.thom_disk_dir);
  }
